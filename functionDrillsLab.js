...
let nameGreeting = nameCheck('Sophie');
...
const bestMovie = (MovieTitle) => {
  return `${MovieTitle} is the best movie ever!`; // ${MovieTitle} will include the value of MovieTitle to the returned string
}
...
function theEliminator(contestants,loser) {
  for(i= 0; i < contestants.length; i++) {
    if(contestants[i].includes(`${loser}`)){
      ...
      i--; //substracting one from the index due to the removed element from loser if the condition is met
  }
}
return contestants
}
...
function emailCheck(email) {
  email = email.trim();  // updated the email string
  if(email.includes('@')) {
...
console.log(emailCheck('michellesaucedasgmail.com'));

////////////////// PROBLEM 19 ////////////////////
/*
  Write a function, naming it whatever you believe to be appropriate, that buys as 
  many chocolate frogs as possible with a certain amount of gold.
...
//CODE HERE
chocolatePrice = 3 //gold
function chocolateFrogs (goldAvailable) {
  return goldAvailable/chocolatePrice
}

let totalFrogs = chocolateFrogs(30);
...
//CODE HERE
function chocolateFrogs2 (goldAvailable) {
  if(goldAvailable % 3 === 0) {
    return goldAvailable/chocolatePrice;
  } else if((goldAvailable-1) % 3 === 0) {
    return (goldAvailable-1) / chocolatePrice;
  }else if((goldAvailable-2) % 3 === 0) {
    return (goldAvailable-2) / chocolatePrice;
}
}
let totalFrogs2 = chocolateFrogs2(75);
